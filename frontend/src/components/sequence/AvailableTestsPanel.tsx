...
export default function AvailableTestsPanel(){
  return (<Card className='p-4 w-full'>
    <div className='flex items-center justify-between mb-2'>
      ...
      <button className='text-sm inline-flex items-center gap-1 px-2 py-1 rounded-lg border'> <Plus className='w-4 h-4'/> Add</button>
    </div>
    ...
    <div className='space-y-3 overflow-y-auto max-h-[60vh] pr-1'>
      {TESTS.map(t=> (
        <div key={t.key} className='rounded-xl border px-3 py-3 bg-white hover:bg-zinc-50 flex items-center justify-between'>
          ...
          <button className='p-1 rounded hover:bg-zinc-100'><MoreVertical className='w-4 h-4'/></button>
        </div>
      ))}
    </div>
    <div className='mt-4'>
