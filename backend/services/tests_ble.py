...
from services.tests_common import (
    ...
    zoom_and_center,  # <-- use the exact helper (peak+center using same methods)
)
...
# ============================== BLE Frequency Accuracy (3-zoom, required order, using zoom_and_center) ==============================
...
async def run_ble_frequency_accuracy_stream(
    *,
    ...
    Required sequence:
      ...
      2) peak search
      3) marker to center
      4) change zoom spectrum params
      5) peak search
      6) marker to center
      7) repeat 4–6 for all zooms
    ...
    try:
        # Analyzer connect + coarse setup (center only; zooms later)
        yield step("connectAnalyzer", "start")
        spec = await ensure_analyzer_async()
        yield step("connectAnalyzer", "done")
        ...
        try:
            ...
            pass
        await asyncio.sleep(0.05)  # tiny settle after centering

        # 4–6) Zoom loop (use the shared helper that does peak+center with same methods)
        total = len(zooms)
        ...
        yield log(
            f"Measured frequency: {_fmt_commas(measured_hz, 2)} Hz "
            f"(Expected={_fmt_commas(expected_hz, 2)} Hz, "
            f"\u0394f={_fmt_commas(err_hz, 2)} Hz, {_fmt_commas(err_ppm, 2)} ppm)"
        )
