...
# Try to access the full YAML (optional; if unavailable, we fall back safely).
try:
...
async def ensure_analyzer_async(timeout: float = DEFAULT_CONNECT_TIMEOUT):
    # Run sync ensure_analyzer() in a thread with timeout
...
async def managed_ble(mac: str):
    """
    Async DUT context with connect/disconnect timeouts.
    The 'finally' uses a shorter timeout so Abort closes fast.
    """
    dut = DUTBLE(mac)
    try:
        ...
        try:
            ...
            # best-effort — never block the stream close
            pass

def get_global_analyzer_ref_offset_db() -> float:
    """
    Reads a top-level analyzer reference offset (in dB) from YAML if present.
    Fallback: 0.0 dB.
    """
    try:
        if load_config is None:
            return 0.0
        cfg = load_config() or {}
        # accept either top-level key or tests.analyzer_analyzer_ref_offset_db
...
async def zoom_and_center(
    ...
    *,
    ...
    """Helper for the 3-zooms Frequency-Accuracy flow."""
    ...
    try:
        # Prefer explicit method name if your driver provides it
...
# ========= Fast non-blocking Abort helpers =========

def _background(coro: "asyncio.Future[Any] | asyncio.Task[Any] | Any") -> None:
    """
    Fire-and-forget task runner with basic shielding, so we never block Abort.
    """
    try:
        if asyncio.iscoroutine(coro):
            asyncio.create_task(coro)
        elif isinstance(coro, (asyncio.Future, asyncio.Task)):
            # already a future/task
            pass
        else:
            # nothing to do
            pass
    except Exception:
        # swallow — background only
        pass
...
async def background_abort_ble(mac: str, *, protocol: str) -> None:
    ...
    try:
        ...
        # background — never raise
        pass

async def background_tidy_spectrum(spec) -> None:
    """
    Optional quick spectrum tidy-up in the background.
    We DO NOT disconnect the LAN session (since you reuse it),
    but you can reset span/VBW/RBW or marker if you want.
    Keep this lightweight and fast with CLOSE_SPEC_TIMEOUT.
    """
    try:
        # example: quick peak search to leave a sane marker; ignore errors
