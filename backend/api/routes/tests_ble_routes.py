...
from services.tests_ble import (
    run_ble_tx_power_stream,
    run_ble_frequency_accuracy_stream,
)
...
def _as_sse(event: dict) -> bytes:
    ...
    Keeps the 'type' field for client-side handling (start, step, log, result, done, error).
    """
...
async def ble_frequency_accuracy_stream(
    ...
    """SSE endpoint for BLE Frequency Accuracy (3 zooms, no explicit CW stop)."""
    try:
